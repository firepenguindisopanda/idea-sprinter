...
interface PrdChatProps {
  ...
  onPRDGenerated?: (prdContent: string) => void;
}

export default function PrdChat({ initialSessionId, prefill: prefillProp, onPhaseChange, onSessionReady, onPRDGenerated }: PrdChatProps) {
  const search = useSearchParams();
  const router = useRouter();
  const prefill = prefillProp ?? search.get("prefill") ?? undefined;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (resp.phase === "complete" && resp.generated_prd) {
        ...
        onPRDGenerated?.(resp.generated_prd);
      }
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto bg-background border-2 border-primary/10 p-4 space-y-3">
      ...
      <div className="sticky bottom-0 bg-background border-t border-primary/10 p-3">
        <div className="flex gap-2 items-start">
          <textarea
            ...
            rows={3}
            className="flex-1 resize-none p-3 border-2 border-primary/10 text-sm"
